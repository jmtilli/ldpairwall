...
void *ctrl_func(void *userdata)
{
  ...
  sin6.sin6_port = htons(args->airwall->conf->port);
  sin.sin_family = AF_INET;
  sin.sin_addr.s_addr = htonl(INADDR_ANY);
  sin.sin_port = htons(args->airwall->conf->port);
  ...
  for (;;)
  {
    ...
    if (datainbuf_get_u32(&inbuf, &payload.local_ip) != 0)
    {
      abort();
    }
    ...
    uint8_t sack_supported, mss_set, sack_set, wscaleshift_set;
    if (datainbuf_get_u8(&inbuf, &sack_supported) != 0)
    {
      abort();
    }
    payload.sack_supported = !!sack_supported;
    if (datainbuf_get_u8(&inbuf, &payload.wscaleshift) != 0)
    {
      abort();
    }
    if (datainbuf_get_u8(&inbuf, &mss_set) != 0)
    {
      abort();
    }
    payload.mss_set = !!mss_set;
    if (datainbuf_get_u8(&inbuf, &sack_set) != 0)
    {
      abort();
    }
    payload.sack_set = !!sack_set;
    if (datainbuf_get_u8(&inbuf, &wscaleshift_set) != 0)
    {
      abort();
    }
    payload.wscaleshift_set = !!wscaleshift_set;
    if (operation & (1<<7))
    {
