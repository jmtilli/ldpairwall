...
ssize_t ssl_name_ctx_feed(struct ssl_name_ctx *ctx, const void *data, size_t sz)
{
  ...
  if (ctx->processed - 3 + sz >= ctx->name_len) // FIXME verify conditional
  {
    if (ctx->processed - 3 + ctx->name_len < sizeof(ctx->nam))
    {
      memcpy(&ctx->nam[ctx->processed - 3], udata, ctx->name_len);
      ctx->nam[ctx->processed - 3 + ctx->name_len] = '\0';
      ctx->truncated = 0;
  ...
  if (ctx->processed - 3 + sz < sizeof(ctx->nam)) // FIXME verify cond
  {
    memcpy(&ctx->nam[ctx->processed - 3], udata, sz);
  }
  else if (sizeof(ctx->nam)-1 > (ctx->processed-3)) // FIXME verify cond
  {
    memcpy(&ctx->nam[ctx->processed - 3], udata,
           sizeof(ctx->nam)-1-(ctx->processed-3));
  }
